...
import { CornerDownRight, CornerUpLeft, LogOut, MessageCircle, PlusIcon, Search, UserCircle, XIcon } from 'lucide-react';
...
const ChatSidebar = ({
    ...
    users, loggedInUser, chats, selectedUser, setSelectedUser, handleLogout , createChat
}: ChatSidebarProps) => {
        ...
        <aside className={`fixed z-20 sm:static top-0 left-0 h-screen w-80 bg-gray-900 border-r border-gray-700 transform ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}
            ...
            <div className='p-6 border-b border-gray-700 '>
                <div className='sm:hidden flex justify-end mb-0'>
                    <button className='p-2 hover:bg-gray-700 rounded-lg transition-colors' onClick={() => setSidebarOpen(false)}><XIcon className='w-5 h-5 text-gray-300' /></button>
                </div>
                ...
                <div className='flex items-center justify-between'>
                    ...
                    <button className={`p-2.5 rounded-lg transition-colors 
                        ...
                        {
                            showAllUsers ? <XIcon className='w-4 h-4' /> : <PlusIcon className='w-4 h-4' />
                        }
                    </button>
                </div>
            </div>
                                        ...
                                        <div className='flex items-center gap-3'>
                                            <div className="relative ">
                                                <div className="w-12 h-12 rounded-full bg-gray-700 flex items-center justify-center">
                                                    ...
                                                    {/* Online user work should be done */}
                                                </div>
                                            </div>
                                            <div className="flex-1 min-w-0 ">
                                                <div className="flex items-center justify-between mb-1">
                                                    ...
                                                    {
                                                        unseenCount > 0 && <div className='bg-red-600 text-white text-xs font-bold 
                                                            rounded-full min-w-[22px] h-5.5 flex items-center justify-center px-2'
                                                        > {unseenCount > 99 ? "99+" : unseenCount}</div>
                                                    }
                                                </div>
            ...
            <div className="p-4 border-t border-gray-700 space-y-2">
                ...
                <button onClick={handleLogout} className='w-full flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-red-600 transition-colors text-red-500 hover:text-white cursor-pointer'>
